...
class BGPPeeringExtension(AttributeExtension):
    ...
    def __init__(self, environment: Environment):
        ...
        Args:
            environment: The environment object passed by the Design Builder.
        ...
        try:
            ...
            from nautobot.ipam.models import IPAddress, IPAddressToInterface  # pylint:disable=import-outside-toplevel
            ...
            self.Device = self.environment.model_factory(Device)  # pylint:disable=invalid-name
            self.Interface = self.environment.model_factory(Interface)  # pylint:disable=invalid-name
            self.IPAddress = self.environment.model_factory(IPAddress)  # pylint:disable=invalid-name
            self._InterfaceModel = Interface  # Direct model for querying
            self._IPAddressModel = IPAddress  # Direct model for querying
            self._IPAddressToInterfaceModel = IPAddressToInterface  # Direct model for querying

            # Log model fields for debugging
            ip_fields = [f.name for f in self._IPAddressModel._meta.get_fields()]
            print(f"Initialized BGPPeeringExtension: InterfaceModel={self._InterfaceModel}, IPAddressModel={self._IPAddressModel}, IPAddress fields={ip_fields}")
            print("Using IPAddressToInterface for IP address querying")
    ...
    def attribute(self, *args, value=None, model_instance: ModelInstance = None) -> None:
        ...
        Design Builder syntax. Additional fields like `source_interface`, `peer_group`,
        and `source_ip.address_family` ('ipv4' or 'ipv6') can be included.

        Args:
            *args: Any additional arguments following the tag name. These are `:` delimited.
            value (dict): Dictionary containing the keys `endpoint_a` and `endpoint_z`.
                Both of these keys must be dictionaries specifying a way to either lookup
                or create the appropriate peer endpoints. Optional keys `source_interface`,
                `peer_group`, and `source_ip.address_family` ('ipv4' or 'ipv6') can be included.
            model_instance: The BGP Peering that is to be updated.

        Raises:
            DesignImplementationError: If the supplied value is not a dictionary
                or it does not include `endpoint_a` and `endpoint_z` as keys, or if
                an interface has multiple IP addresses and `address_family` is not specified.
        ...
        - "!bgp_peering":
              endpoint_a:
                  "!create_or_update:routing_instance__autonomous_system__asn": "65000"
                  "!create_or_update:source_ip":
                      "interface__device__name": "P1"
                      "interface__name": "Loopback0"
                      "address_family": "4"
                  source_interface:
                      "device__name": "P1"
                      "name": "Loopback0"
                  peer_group: "BACKBONE-RR-IPV4-PEERS"
              endpoint_z:
                  "!create_or_update:routing_instance__autonomous_system__asn": "65000"
                  "!create_or_update:source_ip":
                      "interface__device__name": "RR1"
                      "interface__name": "Loopback0"
                      "address_family": "4"
                  source_interface:
                      "device__name": "RR1"
        ...
        - "!bgp_peering":
              endpoint_a:
                  ...
                  source_interface:
                      "device__name": "P1"
                      "name": "Loopback0"
                  peer_group: "BACKBONE-RR-IPV6-PEERS"
              endpoint_z:
                  "!create_or_update:routing_instance__autonomous_system__asn": "65000"
                  "!create_or_update:source_ip":
                      "interface__device__name": "RR1"
        ...
        ```
        """
        print(f"Processing BGP peering with value: {value}")  # Debug
        ...
        # Copy the value to avoid modifying the original
        retval = {**value}
        endpoint_a_data = retval.pop("endpoint_a")
        endpoint_z_data = retval.pop("endpoint_z")

        # Extract source_interface, peer_group, and source_ip
        endpoint_a_source_interface = endpoint_a_data.pop("source_interface", None)
        endpoint_a_peer_group = endpoint_a_data.pop("peer_group", None)
        endpoint_z_source_interface = endpoint_z_data.pop("source_interface", None)
        endpoint_z_peer_group = endpoint_z_data.pop("peer_group", None)

        # Handle source_ip with address_family
        def process_source_ip(endpoint_data):
            source_ip_data = endpoint_data.get("!create_or_update:source_ip", {})
            address_family = source_ip_data.get("address_family")
            if address_family not in [None, "4", "6"]:
                raise DesignImplementationError(
                    f"Invalid address_family '{address_family}' in source_ip. Must be '4' or '6'."
                )

            # Extract device and interface for IP lookup
            device_name = source_ip_data.get("interface__device__name")
            interface_name = source_ip_data.get("interface__name")
            print(f"Processing source_ip: {source_ip_data}")  # Debug
            if device_name and interface_name:
                # Find the interface
                try:
                    interface = self._InterfaceModel.objects.get(
                        device__name=device_name,
                        name=interface_name
                    )
                    print(f"Found interface: {interface}")  # Debug
                except self._InterfaceModel.DoesNotExist:
                    raise DesignImplementationError(
                        f"Interface {interface_name} on device {device_name} not found."
                    )

                # Find associated IP address using IPAddressToInterface
                try:
                    ip_filters = {"ipaddresstointerface__interface": interface}
                    if address_family:
                        ip_filters["ip_version"] = address_family
                    else:
                        ip_filters["ip_version"] = "4"  # Default to ipv4
                        print("No address_family specified; defaulting to 4")  # Debug

                    print(f"Querying IP addresses with filters: {ip_filters}")  # Debug
                    ip_addresses = self._IPAddressModel.objects.filter(**ip_filters)
                    print(f"Found IPs: {ip_addresses}")  # Debug
                    if not ip_addresses:
                        raise DesignImplementationError(
                            f"No IP address found for interface {interface_name} on device {device_name} "
                            f"with address family {address_family or '4'}."
                        )
                    if len(ip_addresses) > 1:
                        raise DesignImplementationError(
                            f"Multiple IP addresses found for interface {interface_name} on device {device_name}. "
                            "Specify 'address_family' as '4' or '6'."
                        )
                    source_ip = ip_addresses[0]
                    endpoint_data["!create_or_update:source_ip"] = {"id": source_ip.id}
                    print(f"Selected IP: {source_ip}")  # Debug
                except MultipleObjectsReturned:
                    raise DesignImplementationError(
                        f"Multiple IP addresses found for interface {interface_name} on device {device_name}. "
                        "Specify 'address_family' as '4' or '6'."
                    ...
                    raise DesignImplementationError(
                        f"Error querying IP addresses for interface {interface_name} on device {device_name}: {str(e)}"
                    )
            return endpoint_data

        endpoint_a_data = process_source_ip(endpoint_a_data)
        endpoint_z_data = process_source_ip(endpoint_z_data)

        # Create PeerEndpoint instances
        endpoint_a = self.PeerEndpoint(self.environment, endpoint_a_data)
        endpoint_z = self.PeerEndpoint(self.environment, endpoint_z_data)

        # Set source_interface and peer_group if provided
        if endpoint_a_source_interface:
            endpoint_a.design_metadata.attributes["source_interface"] = endpoint_a_source_interface
        if endpoint_a_peer_group:
            endpoint_a.design_metadata.attributes["peer_group"] = endpoint_a_peer_group
        if endpoint_z_source_interface:
            endpoint_z.design_metadata.attributes["source_interface"] = endpoint_z_source_interface
        if endpoint_z_peer_group:
            endpoint_z.design_metadata.attributes["peer_group"] = endpoint_z_peer_group
        ...
        # Handle existing peerings to prevent duplicates
        ...
        def post_save():
            ...
            endpoint_a_instance = peering_instance.design_instance.endpoint_a
            endpoint_z_instance = peering_instance.design_instance.endpoint_z
            endpoint_a_instance.peer, endpoint_z_instance.peer = endpoint_z_instance, endpoint_a_instance

            # Apply source_interface and peer_group to the saved instances
            if endpoint_a_source_interface:
                endpoint_a_instance.source_interface = endpoint_a_source_interface
            if endpoint_a_peer_group:
                endpoint_a_instance.peer_group = endpoint_a_peer_group
            if endpoint_z_source_interface:
                endpoint_z_instance.source_interface = endpoint_z_source_interface
            if endpoint_z_peer_group:
                endpoint_z_instance.peer_group = endpoint_z_peer_group

            endpoint_a_instance.save()
            endpoint_z_instance.save()
