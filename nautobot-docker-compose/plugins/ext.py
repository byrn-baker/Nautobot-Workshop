...
from nautobot.ipam.models import Prefix, IPAddress  # Added IPAddress
...
def resolve_ipaddress_query(query):
    """
    Convert legacy interface lookups for IPAddress into the modern assigned_object_type/assigned_object_id pattern.
    Modifies the query dict in place.
    """
    interface_fields = {"interface__device__name", "interface__name"}
    if any(k in query for k in interface_fields):
        # Extract interface lookup parameters
        interface_kwargs = {}
        for k in list(query.keys()):
            if k.startswith("interface__"):
                interface_kwargs[k.replace("interface__", "")] = query.pop(k)
        # Lookup the interface
        try:
            interface = dcim.Interface.objects.get(**interface_kwargs)
        except dcim.Interface.DoesNotExist:
            raise DesignImplementationError(f"No interface found for lookup {interface_kwargs}")
        # Set the assigned_object_type/id for IPAddress query
        query["assigned_object_type"] = ContentType.objects.get_for_model(dcim.Interface)
        query["assigned_object_id"] = interface.id
    return query
...
class LookupExtension(AttributeExtension, LookupMixin):
    ...
    def attribute(self, *args, value, model_instance) -> None:  # pylint:disable=arguments-differ
        ...
        # If model_class is IPAddress, translate interface__* lookups
        if model_name == "ipaddress":
            query = resolve_ipaddress_query(query)
